...
export function useDashboardOverview() {
  ...
  const applyOverview = useCallback((nextOverview: OverviewResponse, markUpdated = false) => {
  ...
  const loadOverview = useCallback(async () => {
    setRequestStatus((current) => (current === "idle" ? "loading" : current));
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      socket.onmessage = (event) => {
        ...
        lastMessageAt = Date.now();
        setRealtimeStatus("live");
        applyOverview(message.payload, message.type === "overview.updated");
      };
    ...
    loadOverview();
    initialConnectTimer = window.setTimeout(connect, INITIAL_CONNECT_DELAY_MS);
