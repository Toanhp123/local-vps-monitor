...
const createAppHealthIncident = (
  ...
  return {
    id: incidentId(payload.serverId, "app-health", app.id, observedAt, app.health),
    appId: app.id,
    appName: app.name,
    ...
    title: isRecovered ? `${app.name} recovered` : `${app.name} is ${currentLabel.toLowerCase()}`
  };
};
...
const createAppRestartIncident = (
  ...
  return {
    id: incidentId(payload.serverId, "app-restart", app.id, observedAt, String(currentRestarts)),
    appId: app.id,
    appName: app.name,
...
const createAppAddedIncident = (
  ...
  return {
    id: incidentId(payload.serverId, "app-added", app.id, observedAt, app.health),
    appId: app.id,
    appName: app.name,
    currentHealth: app.health,
    kind: "app-added",
    ...
    title: isHealthy ? `${app.name} added` : `${app.name} added with ${currentLabel.toLowerCase()} status`
  };
};
  ...
  for (const app of apps) {
    ...
    const healthIncident =
      previousApp || !previousServer
        ...
        : null;
    const restartIncident = createAppRestartIncident(app, previousApp, payload, observedAt);
  ...
  if (previousServer) {
    for (const previousApp of previousApps) {
      if (!currentById.has(previousApp.id)) {
        incidents.push(createAppRemovedIncident(previousApp, payload, observedAt));
      }
    }
  }
...
const withIncidentTimeline = (
  ...
  return [...incidents, ...(previousIncidents ?? [])].slice(0, maxIncidentEvents);
};
...
export const createStoredServerFromSnapshot = (
  ...
  const incidents = createIncidentEvents(apps, payload, previousServer, receivedAt);
