...
import type { SshTarget, SshTargetBootstrapInput, SshTargetCreateInput } from "../../shared/types";
import { addKnownHostKey, verifyKnownHostKey } from "../integrations/ssh/sshKnownHosts";
...
type PasswordTarget = Pick<SshTargetBootstrapInput, "host" | "port" | "username">;
...
const connectWithPassword = (
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      cleanup();
      client.end();
      reject(new Error(`SSH password setup timed out after ${timeoutMs}ms`));
    }, timeoutMs);
    ...
    client.connect({
      ...
      hostVerifier: (key: Buffer) => {
        ...
        if (verification.status === "missing-file" || verification.status === "not-found") {
          return true;
        }
...
export class SshTargetBootstrapService {
  ...
  async bootstrapTarget(input: SshTargetBootstrapInput) {
    ...
    const passwordConnection = await connectWithPassword(input, input.password, this.sshCommandTimeoutMs);
    ...
    try {
      ...
      if (!installResult.ok) {
        const output = [installResult.stderr.trim(), installResult.stdout.trim()].filter(Boolean).join("\n");
        throw new Error(`Failed to install monitor public key.${output ? ` ${output}` : ""}`);
      }
    } finally {
    ...
    const keyClient = await connectSshTarget(keyTestTarget, this.sshCommandTimeoutMs);

    try {
      const keyTestResult = await runSshCommand(keyClient, "echo ok", this.sshCommandTimeoutMs);
      if (!keyTestResult.ok) {
        throw new Error(`Monitor key test failed: ${keyTestResult.stderr || keyTestResult.stdout}`);
      }
    } catch (error) {
      throw new Error(`Monitor key was installed, but key login failed: ${errorMessage(error)}`);
    } finally {
      keyClient.end();
    }
