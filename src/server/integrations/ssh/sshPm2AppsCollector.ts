...
const pm2Health = (status?: string): HealthStatus => {
	...
	if (normalized === "launching" || normalized === "stopping")
		return "warning";

	return "unknown";
};

export const collectSshPm2Apps = async (
	client: Client,
	timeoutMs: number,
): Promise<AppSnapshot[]> => {
	const result = await safeRunSshCommand(
		client,
		"command -v pm2 >/dev/null 2>&1 && pm2 jlist || true",
		timeoutMs,
	);
	...
	return rows.map((row) => {
		const uptimeMs = row.pm2_env?.pm_uptime
			? Date.now() - row.pm2_env.pm_uptime
			: undefined;
		...
		return {
			...
			cpuPercent:
				typeof row.monit?.cpu === "number" ? row.monit.cpu : undefined,
			memoryBytes:
			...
			uptimeSeconds: uptimeMs
				? Math.max(0, Math.round(uptimeMs / 1000))
				: undefined,
			raw: {
				pmId: row.pm_id,
