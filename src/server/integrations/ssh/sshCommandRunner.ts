...
const { Client, utils } = createRequire(import.meta.url)(
	"ssh2",
...
const validatePrivateKeyPathValue = (filePath: string) => {
	...
	if (
		...
		throw new Error(
			"Private key path must be a local file path, not pasted key content.",
		);
	}
};
...
const readPrivateKey = (filePath: string) => {
	...
	if (!parsedKey.isPrivateKey()) {
		throw new Error(
			"Private key file points to a public key. Use the private key file path without the .pub suffix.",
		);
	}
