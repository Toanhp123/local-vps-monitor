...
const writePublicKeyFromPrivateKey = async (privateKeyPath: string, publicKeyPath: string) => {
  ...
  if (!publicKeyPrefixPattern.test(publicKey)) {
    throw new Error(`Cannot derive monitor public key from ${privateKeyPath}`);
  }
...
export const ensureMonitorKeyPair = async (): Promise<MonitorKeyPair> => {
  ...
  if (!fs.existsSync(privateKeyPath)) {
    if (fs.existsSync(publicKeyPath)) {
      throw new Error(`Monitor public key exists without private key: ${publicKeyPath}`);
    }

    await execFileAsync(
      "ssh-keygen",
      ["-t", "ed25519", "-f", privateKeyPath, "-N", "", "-C", "vps-monitor"],
      {
        encoding: "utf8",
        timeout: 12_000,
        windowsHide: true
