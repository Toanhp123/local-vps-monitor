...
export const collectSshHostMetrics = async (client: Client, timeoutMs: number): Promise<HostMetrics> => {
  const [hostname, platform, arch, uptime, cpuCount, loadAverage, memInfo] = await Promise.all([
    safeRunSshCommand(client, "hostname 2>/dev/null || uname -n 2>/dev/null || echo unknown", timeoutMs),
    safeRunSshCommand(client, "uname -s 2>/dev/null || echo linux", timeoutMs),
    safeRunSshCommand(client, "uname -m 2>/dev/null || echo unknown", timeoutMs),
    safeRunSshCommand(client, "cat /proc/uptime 2>/dev/null || echo 0", timeoutMs),
    safeRunSshCommand(client, "nproc 2>/dev/null || getconf _NPROCESSORS_ONLN 2>/dev/null || echo 1", timeoutMs),
    safeRunSshCommand(client, "cat /proc/loadavg 2>/dev/null || echo '0 0 0'", timeoutMs),
    safeRunSshCommand(client, "cat /proc/meminfo 2>/dev/null || true", timeoutMs)
  ]);
